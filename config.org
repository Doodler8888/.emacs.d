...
#+begin_src emacs-lisp
  ...
  (defun fzf-from-root-no-prompt ()
    ...
    (interactive)
    (setenv "FZF_DEFAULT_COMMAND" "fd --type f --hidden --follow --exclude .git --exclude .snapshots --exclude opt --exclude lib --exclude lib64 --exclude mnt --exclude proc --exclude run --exclude sbin --exclude srv --exclude sys --exclude tmp . /")
    (fzf))
