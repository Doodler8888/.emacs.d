...
(defun my-eshell-fullscreen ()
  ...
  (interactive)
  (let* ((tab-index (my-current-tab-index))
         (eshell-buffer-name (format "*eshell<%d>*" tab-index)))
    (if tab-index
        (progn
          (puthash tab-index (current-window-configuration) my-saved-tab-configurations)
          (message "Saved window configuration for tab index: %d" tab-index)
          (delete-other-windows)
      ...
      (message "Failed to get tab index. Is the tab-bar-mode enabled?"))))

(defun my-restore-window-configuration ()
  "Restore the previously saved window configuration for the current tab."
  (interactive)
  (let* ((tab-index (my-current-tab-index))
         (config (gethash tab-index my-saved-tab-configurations)))
    (if config
        (progn
          (set-window-configuration config)
          (message "Restored window configuration for tab index: %d" tab-index))
      (message "No saved window configuration for this tab index: %d" tab-index))))
