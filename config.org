...
#+begin_src emacs-lisp
  ...
  ;; ;; Define sql-connection-alist before using it
  ;; (defvar sql-connection-alist nil
  ;; "Alist of SQL connection configurations.")

  ;; ;; Define a macro to simplify the creation of new connections
  ;; (defmacro define-sql-connection (name product user server port database)
  ;; `(push (list ',name
  ;;               (cons 'sql-product ',product)
  ;;               (cons 'sql-user ',user)
  ;;               (cons 'sql-server ',server)
  ;;               (cons 'sql-port ',port)
  ;;               (cons 'sql-database ',database))
  ;;         sql-connection-alist))

  ;; ;; Use the macro to define your connections
  ;; (define-sql-connection postgres-wurfkreuz postgres "wurfkreuz" "localhost" 5432 "wurfkreuz")
  ;; (define-sql-connection postgres-shelf postgres "wurfkreuz" "localhost" 5432 "shelf")

  ;; (defun my-sql-connect (connection)
  ;; "Connect to a SQL database (specified by CONNECTION)."
  ;; (let ((params (assoc connection sql-connection-alist)))
  ;;     (if params
  ;;         (let ((sql-product (cdr (assoc 'sql-product params)))
  ;;               (sql-user (cdr (assoc 'sql-user params)))
  ;;               (sql-server (cdr (assoc 'sql-server params)))
  ;;               (sql-port (cdr (assoc 'sql-port params)))
  ;;               (sql-database (cdr (assoc 'sql-database params))))
  ;;           (sql-connect connection))
  ;;       (message "Connection details not found for: %s" connection))))

  ;; (defun my-sql-select-database ()
  ;; "Prompt the user to select a database connection defined in `sql-connection-alist`."
  ;; (interactive)
  ;; (let* ((choices (mapcar (lambda (item) (symbol-name (car item))) sql-connection-alist))
  ;;         (choice (completing-read "Select database: " choices nil t)))
  ;;     (my-sql-connect (intern choice))))
  ...
  (setq sql-connection-alist
        '((postgres-wurfkreuz
          ...
          (sql-port 5432)
          ;; (sql-password (getenv "POSTGRES_WURFKREUZ_PASSWORD"))
          ...
          (sql-server "localhost")
          (sql-port 5432)
          ;; (sql-password (getenv "POSTGRES_SHELF_PASSWORD"))
          (sql-database "shelf"))))
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-s C-q") 'my-sql-select-database)
