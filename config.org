...
#+begin_src emacs-lisp
  ...
  (defun popper-async-shell-command (input)
    "Popper window specifically with the async shell command buffer."
    (interactive "MAsync shell command: ") ; Prompt for command
    (async-shell-command input)
    (popper-toggle)
    (switch-to-buffer "*Async Shell Command*"))

  (funcall-interactively #'popper-async-shell-command)
  ...
  ;; (defun popper-message ()
  ;;   "Popper window with the message buffer"
  ;;   (interactive)
  ;;   (unless (get-buffer-window "*Messages*") ;; Check if buffer is displayed
  ;;     (switch-to-buffer "*Messages*"))
  ;;   (popper-toggle))
