...
#+begin_src emacs-lisp
  ...
  (defun save-desktop-with-name ()
    "Save the current desktop session with a custom name."
    (interactive)
    (let ((desktop-dir (concat user-emacs-directory "desktop/")))
      ...
      (let* ((session-name (read-string "Session name: "))
            (session-dir (concat desktop-dir session-name "/")))
  ...
  (defun save-current-desktop ()
    ...
    (if desktop-dirname
      ...
      (message "No desktop session name set!")))

  (defun my-desktop-clear ()
    "Clear the current desktop session without confirmation."
    (interactive)
  ...
  (defun load-desktop-with-name ()
    "Load a saved desktop session by name from the 'desktop' directory."
    (interactive)
    (my-desktop-clear)
    (let ((desktop-dir (concat user-emacs-directory "desktop/")))
      (unless (file-exists-p desktop-dir)
        (error "No desktop directory found"))
      (let* ((dirs (remove "." (remove ".." (directory-files desktop-dir t directory-files-no-dot-files-regexp))))
            (session-dir (completing-read "Load session: " dirs nil t)))
