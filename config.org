...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'tramp
    (let ((sudo-method (assoc "sudo" tramp-methods)))
      (when sudo-method
        (let ((session-timeout (assoc 'tramp-session-timeout (cdr sudo-method))))
          (if session-timeout
              ;; If the tramp-session-timeout entry exists, just set its value.
              ;; Ensure the value is a list.
              (setcdr session-timeout '(1))
            ;; Otherwise, add the tramp-session-timeout entry.
            ;; Again, ensure the value is specified as a list.
            (setcdr sudo-method (cons '(tramp-session-timeout 1) (cdr sudo-method))))))))

  ;; (defun clear-authinfo-and-reset ()
  ;;   "Clears the .authinfo file and resets password cache and TRAMP sessions."
  ;;   (let* ((authinfo-file (expand-file-name "~/.authinfo"))
  ;;         (expiry-seconds 10) ;; Expiry time in seconds.
  ;;         (current-time (current-time))
  ;;         (last-modified (nth 5 (file-attributes authinfo-file)))
  ;;         (time-difference (float-time (time-subtract current-time last-modified))))
  ;;     (when (> time-difference expiry-seconds)
  ;;       (tramp-cleanup-all-connections)
  ;;       (message "Authentication info and sessions reset due to expiry."))))

  ;; ;; Optionally, set up a periodic check.
  ;; (run-with-timer 0 10 'clear-authinfo-and-reset) ;; Check every 60 seconds.

  ;; (setq auth-sources '("~/.authinfo"))

  ;; (run-with-timer 0 10 (lambda () (password-cache-remove '("sudo" "archlinux" "wurfkreuz" "sudo"))))
