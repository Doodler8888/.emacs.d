...
(with-eval-after-load 'eshell
  (with-eval-after-load 'evil
    ...
    (evil-define-key 'normal eshell-mode-map (kbd "M-l") 'eshell-clear-buffer)))
...
;; (defun check-and-switch-to-docker-or-kubernetes-tab ()
;;   "Check if 'docker' or 'kubernetes' tabs exist and switch to them if they do."
;;   (interactive)
;;   (let* ((tabs (funcall tab-bar-tabs-function))
;;          (docker-tab (cl-find-if (lambda (tab) (string= (alist-get 'name tab) "docker")) tabs))
;;          (kubernetes-tab (cl-find-if (lambda (tab) (string= (alist-get 'name tab) "kubernetes")) tabs)))
;;     (cond
;;      ;; If Docker tab exists, switch to it and activate its function
;;      ((and docker-tab)
;;       (tab-bar-switch-to-tab-by-id (alist-get 'id docker-tab))
;;       (message "Switched to Docker tab")
;;       (docker-template)
;;       ;; After activating Docker tab, check for Kubernetes tab
;;       (when kubernetes-tab
;;         (tab-bar-switch-to-tab-by-id (alist-get 'id kubernetes-tab))
;;         (message "Switched to Kubernetes tab")
;;         (kubernetes-overview)))
;;      ;; If only Kubernetes tab exists, switch to it and activate its function
;;      ((and kubernetes-tab (not docker-tab))
;;       (tab-bar-switch-to-tab-by-id (alist-get 'id kubernetes-tab))
;;       (message "Switched to Kubernetes tab")
;;       (kubernetes-overview))
;;      ;; If neither tab exists
;;      (t (message "No Docker or Kubernetes tab found")))))

;; ;; Add the modified function to desktop-after-read-hook
;; (add-hook 'desktop-after-read-hook #'check-and-switch-to-docker-or-kubernetes-tab)
