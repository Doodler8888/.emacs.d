...
(defun my-vc-branch ()
  ...
  (let* ((git-cmd "git rev-parse --abbrev-ref HEAD 2>/dev/null")
...
(defun my-git-commit-setup ()
  ...
  (insert "GG-1729 Add: Создание пайплайна и Dockerfile для выполнения задания.\n"))

(add-hook 'git-commit-setup-hook 'my-git-commit-setup)

(defvar my-default-branch-name "2409/agud/1730_pipeline_template"
  "Default name for new branches.")

(defun my-magit-read-branch-args (prompt &optional secondary-default)
  "Read branch name with a default value.
PROMPT and SECONDARY-DEFAULT are passed to `magit-branch-read-args'."
  (let ((magit-branch-read-upstream-first nil))
    (magit-branch-read-args prompt secondary-default my-default-branch-name)))

(setq magit-branch-read-args-function #'my-magit-read-branch-args)
...
;; (setq tramp-ssh-controlmaster-options (format "-i %s" "~/.ssh/wireguard2.pem"))

;; (setq tramp-default-method "ssh")  ; Make sure SSH is the default method

;; (add-to-list 'tramp-default-user-alist '("sudo" "3.71.6.56" "root"))
