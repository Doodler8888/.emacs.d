...
#+begin_src emacs-lisp
  ...
  ;;   (add-to-list 'completion-at-point-functions #'my-eshell-chmod-completion))
  ...
  ;; ;; Define custom completion function for chmod
  ;; (defun my-eshell-chmod-completion ()
  ;;   "Provide completion for the `chmod` command in Eshell."
  ;;   (when (and (eq major-mode 'eshell-mode)
  ;;             (save-excursion
  ;;               (eshell-bol)
  ;;               (looking-at-p "chmod ")))
  ;;     (let* ((point (point))
  ;;           (begin (save-excursion (eshell-bol) (search-forward "chmod ") (point)))
  ;;           (end (point))
  ;;           (completions '("400" "500" "600" "644" "755" "777")))
  ;;       (list begin end completions))))

  (defun my-eshell-chmod-completion ()
    "Provide dynamic completion for the `chmod` command in Eshell."
    (when (and (eq major-mode 'eshell-mode)
              (save-excursion
                (eshell-bol)
                (looking-at-p "chmod ")))
      (let* ((point (point))
            ...
            (initial-completions '("a" "g" "o" "u" "+" "-" "="))
            (second-level-completions '("X" "g" "o" "r" "s" "t" "u" "w" "x"))
            completions)
        (setq completions
              (cond
              ;; Provide initial completions
              ((string-match-p "\\`chmod +[agou+-=]*\\'" input)
                initial-completions)
              ;; Provide second level completions
              ((string-match-p "\\`chmod +\\([agou+-=]+\\)$" input)
  ...
  (provide 'my-eshell-chmod-completion)
