...
;;   "Check the status the daemon with NAME."
;;   (interactive
;;    (list
;;     (daemons--completing-read "status")))
;;   (daemons--run-action 'status name))
...
;; (defun my/postgres-list-roles ()
;;   "Return a list of PostgreSQL roles without establishing a connection."
;;   (let* ((cmd "psql -Atq -c \"SELECT rolname FROM pg_roles;\"")
...
(defun sql-postgres (role database server port)
...
(defun psql ()
  ...
  (let* ((selected-role (my/select-postgres-role))
    ...
    (sql-postgres selected-role selected-database selected-server selected-port)))

;; (defun psql-session ()
;;   "Open sql buffer with psql shell"
;;   (interactive)
;;   (let ((psql-buffer (call-interactively 'psql)))
...
(defun psql-session ()
  ...
  (interactive)
  (let* ((temp-sql-file (make-temp-file "psql-session-" nil ".sql"))
...
;; Optionally, add a hook to delete the temporary file when the buffer is killed
(add-hook 'kill-buffer-hook
          (lambda ()
            (when (and buffer-file-name
                       (string-match-p "psql-session-.*\\.sql$" buffer-file-name))
