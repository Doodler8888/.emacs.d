...
(setq sql-connection-alist
      '((my-postgres
         ...
         (sql-user "<your_default_user>")
         (sql-database "<your_default_database>")
         (sql-server "<your_server>")
         (sql-port "<your_port>"))))
...
(defun sql-postgres (role database)
...
(defun my/select-role-and-database-and-login ()
  "Prompt for a PostgreSQL role and then a database owned by that role, then log into sql-interactive-mode."
  (interactive)
  (let* ((selected-role (my/select-postgres-role))
         (selected-database (my/select-postgres-database selected-role)))
    (sql-postgres selected-role selected-database)))
