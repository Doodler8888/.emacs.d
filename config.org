...
;; This code attempts to delete additional font-lock usage in lisp modes inside comment lines like when i put some word in single quotes.
(defun force-comment-face ()
  "Force everything after semicolon to use comment face, but only when semicolon is the first non-whitespace character."
  (font-lock-add-keywords
   nil
   '(("^[[:space:]]*;.*$" 0 font-lock-comment-face prepend))
   'append))

;; Add to lisp mode hooks
(add-hook 'emacs-lisp-mode-hook 'force-comment-face)
