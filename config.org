...
#+begin_src emacs-lisp
  ...
  (use-package evil-collection
    ...
    :config
    (setq evil-collection-mode-list '(dashboard eshell dired wdired ibuffer org term ansi lsp-ui-imenu elpaca minibuffer))
...
#+begin_src emacs-lisp
  ...
  (use-package general
    ...
    (w/leader-keys
      ...
      "ep" '(eshell-new-pop :wk "Spawn a new eshell buffer in a popper window")
...
#+begin_src emacs-lisp
  ...
  (defun senv ()
    "Activate Python virtual environment located in ./.venv."
    (interactive)
    (let ((venv-path (expand-file-name "./.venv")))
      ...
      (message "Activated virtual environment in %s" venv-path)))

  (defun my-eshell-prompt ()
    "Custom Eshell prompt that shows the current Python virtual environment if activated."
    (concat
    (if pyvenv-virtual-env-name
        (format "[env] " pyvenv-virtual-env-name)  ; Display the active virtual environment name
      "")
    (eshell/pwd)  ; Display the current directory
    (if (= (user-uid) 0) " # " " $ ")))  ; Display prompt character based on user privileges
...
#+begin_src emacs-lisp
  ...
  (use-package pyvenv
    :config
    (pyvenv-mode 1)  ; Enable pyvenv mode to manage Python virtual environments
  )
...
#+begin_src emacs-lisp
  ...
  (add-hook 'go-ts-mode-hook #'my/disable-flycheck-in-emacs-lisp-mode)
  (add-hook 'python-ts-mode-hook #'my/disable-flycheck-in-emacs-lisp-mode)
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-mode
    ...
    (lsp-register-client
     (make-lsp-client :new-connection (lsp-stdio-connection "emacs-lsp-booster -- pyright")
